...
2. For playback from an offset, create **zero-copy slice buffers** over the routed buffer using `AVAudioPCMBuffer(pcmFormat:bufferListNoCopy:deallocator:)` (macOS 10.15+): build an `AudioBufferList` whose `mData` pointers are the routed buffer's channel pointers advanced by `startFrame`, with `mDataByteSize = frameCount * 4`. Pass `deallocator: nil` (the routed buffer owns the memory and outlives the slice — it's retained by `Track`). Schedule the slice exactly like the old scratch buffer, including `.loops` for the loop region.
3. If the zero-copy wrapper proves fiddly, acceptable fallback: keep one memcpy per *user-initiated* seek (not per drag event — see 2.2) into a single scratch buffer, but the load-time routing must still land so the copy is a straight same-format memcpy.
