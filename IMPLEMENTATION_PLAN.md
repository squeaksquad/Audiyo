...
2. Compute the shared start time **after** all scheduling is done. **SUPERSEDED (2026-07-06): use the host-time anchor from 1.1, NOT the sample-time approach below.** The engine's sample counter resets across engine restarts (song switches, device changes) and `lastRenderTime` can return a stale pre-restart value that still passes `isSampleTimeValid`, desyncing the playhead from the audio (bug observed in practice). Original sample-time sketch kept for the record:
   ```swift
...
- 2026-07-06 — SEEK-HIJACK FIX (user-reported: "seek sometimes jumps the playhead forward and plays from there"): the loop-handle circles were always rendered on top of the timeline — at default loopStart=0/loopEnd=1 they sit at the far edges — and stole clicks/drags landing within ~8pt, then their `onEnded` called `restartIfPlaying()` unconditionally, replaying from the current position instead of the clicked one. Handles now render only while `isLooping` is on, and their drag-end restart is guarded by `isLooping` like the keyboard loop commands.
- 2026-07-06 — PLAYHEAD-DESYNC FIX (user-reported: after switching songs, seeking desyncs the playhead from the clicked spot): `play(from:)` anchored the shared start time to `outputNode.lastRenderTime.sampleTime`, but the engine's sample counter resets across engine restarts (song switches rebuild the engine in `setupTracks`) and can return a stale pre-restart value that still passes `isSampleTimeValid` — players then start against the wrong epoch and `playerTime(forNodeTime:)` reports offset frame counts. Start time is now always anchored to the mach host clock (`AVAudioTime(hostTime:)` with proper tick conversion), which never resets; all players still share one identical `AVAudioTime`, so inter-track sample lock is unchanged. **Do not reintroduce sample-time anchoring** (this supersedes the 1.2 guidance preferring sample time). Also clamped negative `playerTime.sampleTime` during the pre-start arm window in `updateProgress`.
...
- 2026-09-15 — Playback core replaced: the per-transport `makeSlice` hardware-format buffers (frames × device channels per stem, multi-GB on wide devices, zero-filled on the main thread before every play/seek) are gone. A single `AVAudioSourceNode` render block now mixes every stem's mono buffer straight into its routed output channel from a shared position (`TransportState`, `os_unfair_lock`). Play/seek/marker jumps, loop edits, routing and volume changes are plain state writes picked up on the next render cycle; sync across stems is inherent since one callback renders all of them. Meters are computed in the render block. `AVAudioPlayerNode`s, `scheduleGeneration`, host-time start anchoring and the meter taps were removed with it.
